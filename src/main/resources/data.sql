...
INSERT INTO cafe_article (writer, title, contents, created_time, modified_time, deleted)
    VALUES ((SELECT user_id FROM cafe_user WHERE user_id = 'mylumiere'), '삽질킹을 아시나요?', '저는 삽질만 합니다.', now(), now(), false),
           ((SELECT user_id FROM cafe_user WHERE user_id = 'chance'), '이건 무슨 질문일까요?', '과연 이건 무슨 질문일까요?', now(), now(), false),
           ((SELECT user_id FROM cafe_user WHERE user_id = 'mylumiere'), '과연 세 개는 나올까요?', '세 개도 나오겠지요.', now(), now(), false);
INSERT INTO cafe_reply (article_id, writer, contents, created_time, modified_time, deleted)
    VALUES ((SELECT id from cafe_article WHERE id = '1'), (SELECT user_id FROM cafe_user WHERE user_id = 'chance'),
            '아뇨, 저는 잘 모릅니다.', now(), now(), false),
