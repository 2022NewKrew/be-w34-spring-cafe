...
public class ReplyController {
    ...
    @PutMapping("/{replyId}")
    public String updateReply(@PathVariable(value="id") Long articleId, @PathVariable Long replyId, @RequestParam String updateContent, @SessionAttribute(name= SessionConst.LOGIN_COOKIE) User user, Model model) {
        try{
    ...
    public String deleteReply(@PathVariable(value="id") Long articleId, @PathVariable Long replyId, @SessionAttribute(name= SessionConst.LOGIN_COOKIE) User user, Model model) {
        try{
            replyService.delete(replyId, user.getId());
            return "redirect:/articles/" + articleId;
        } catch (IsNotAuthorOfThisComment e) {
