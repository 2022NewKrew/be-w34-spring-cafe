...
### 구현 및 작업 내용
1. docker
   - mysql 8.0 기본 설정 후 이미지 생성
   - 생성한 이미지로 mysql8 컨테이너 띄운 후 프로젝트에 연결
2. 댓글 기능 구현
   - 자신의 댓글에만 삭제 버튼 표시
   - 서비스 레이어에서 세션 아이디로 권한 체크
3. 리뷰 내용 반영
   - 업데이트 하는 request의 method를 post에서 put으로 변경
   - Sql을 enum으로 관리 및 user, article, reply 세 개로 나누어 관리
